// 窗口缩放功能演示程序

use mira::window::WindowManager;
use winit::{
    dpi::PhysicalPosition,
    event::{Event, WindowEvent},
    event_loop::EventLoop,
};
use log::{info, error};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    // 初始化日志系统
    env_logger::init();
    
    info!("窗口缩放演示程序启动");
    
    // 创建事件循环
    let event_loop = EventLoop::new()?;
    
    // 创建窗口管理器
    let mut window_manager = match WindowManager::new(&event_loop) {
        Ok(manager) => {
            info!("窗口管理器创建成功");
            manager
        }
        Err(e) => {
            error!("窗口管理器创建失败: {}", e);
            return Err(e.into());
        }
    };
    
    info!("窗口初始状态:");
    info!("  位置: {:?}", window_manager.position());
    info!("  尺寸: {:?}", window_manager.size());
    info!("  旋转角度: {:.1}°", window_manager.rotation());
    
    info!("操作说明:");
    info!("  - 鼠标滚轮向上: 放大窗口 (+10%)");
    info!("  - 鼠标滚轮向下: 缩小窗口 (-10%)");
    info!("  - 左键拖拽: 移动窗口");
    info!("  - 关闭窗口: 退出程序");
    
    // 存储当前鼠标位置
    let mut current_cursor_pos = PhysicalPosition::new(0.0, 0.0);
    
    // 运行事件循环
    event_loop.run(move |event, event_loop| {
        match event {
            Event::WindowEvent {
                event: WindowEvent::CloseRequested,
                ..
            } => {
                info!("收到窗口关闭请求");
                event_loop.exit();
            }
            Event::WindowEvent {
                event: WindowEvent::CursorMoved { position, .. },
                ..
            } => {
                // 更新当前鼠标位置
                current_cursor_pos = position;
                
                // 如果正在拖拽，更新窗口位置
                if window_manager.is_dragging() {
                    window_manager.update_drag(position);
                }
            }
            Event::WindowEvent {
                event: WindowEvent::MouseInput { state, button, .. },
                ..
            } => {
                use winit::event::{ElementState, MouseButton};
                
                match (state, button) {
                    (ElementState::Pressed, MouseButton::Left) => {
                        // 开始拖拽，使用当前鼠标位置
                        window_manager.start_drag(current_cursor_pos);
                        info!("开始拖拽，鼠标位置: {:?}", current_cursor_pos);
                    }
                    (ElementState::Released, MouseButton::Left) => {
                        window_manager.end_drag();
                        info!("结束拖拽，最终位置: {:?}", window_manager.position());
                    }
                    _ => {}
                }
            }
            Event::WindowEvent {
                event: WindowEvent::MouseWheel { delta, .. },
                ..
            } => {
                use winit::event::MouseScrollDelta;
                
                let old_size = window_manager.size();
                let old_aspect_ratio = old_size.width as f32 / old_size.height as f32;
                
                match delta {
                    MouseScrollDelta::LineDelta(_, y) => {
                        if y > 0.0 {
                            // 向上滚动，放大 10%
                            window_manager.scale(1.1);
                            let new_size = window_manager.size();
                            let new_aspect_ratio = new_size.width as f32 / new_size.height as f32;
                            
                            info!("窗口放大 10%:");
                            info!("  尺寸: {:?} -> {:?}", old_size, new_size);
                            info!("  宽高比: {:.3} -> {:.3}", old_aspect_ratio, new_aspect_ratio);
                            
                            // 验证宽高比是否保持
                            let aspect_ratio_diff = (old_aspect_ratio - new_aspect_ratio).abs();
                            if aspect_ratio_diff > 0.01 {
                                info!("  警告: 宽高比发生变化，差异: {:.3}", aspect_ratio_diff);
                            } else {
                                info!("  ✓ 宽高比保持不变");
                            }
                        } else if y < 0.0 {
                            // 向下滚动，缩小 10% (1/1.1 ≈ 0.909)
                            window_manager.scale(1.0 / 1.1);
                            let new_size = window_manager.size();
                            let new_aspect_ratio = new_size.width as f32 / new_size.height as f32;
                            
                            info!("窗口缩小 10%:");
                            info!("  尺寸: {:?} -> {:?}", old_size, new_size);
                            info!("  宽高比: {:.3} -> {:.3}", old_aspect_ratio, new_aspect_ratio);
                            
                            // 验证宽高比是否保持
                            let aspect_ratio_diff = (old_aspect_ratio - new_aspect_ratio).abs();
                            if aspect_ratio_diff > 0.01 {
                                info!("  警告: 宽高比发生变化，差异: {:.3}", aspect_ratio_diff);
                            } else {
                                info!("  ✓ 宽高比保持不变");
                            }
                        }
                    }
                    MouseScrollDelta::PixelDelta(delta) => {
                        // 处理像素级滚动（触摸板等）
                        let y = delta.y as f32;
                        if y.abs() > 10.0 {  // 增加阈值避免过于敏感
                            if y > 0.0 {
                                // 向上滚动，放大 10%
                                window_manager.scale(1.1);
                                let new_size = window_manager.size();
                                info!("窗口放大 10% (触摸板)，尺寸: {:?} -> {:?}", old_size, new_size);
                            } else {
                                // 向下滚动，缩小 10%
                                window_manager.scale(1.0 / 1.1);
                                let new_size = window_manager.size();
                                info!("窗口缩小 10% (触摸板)，尺寸: {:?} -> {:?}", old_size, new_size);
                            }
                        }
                    }
                }
            }
            Event::WindowEvent {
                event: WindowEvent::RedrawRequested,
                ..
            } => {
                // 这里将来会调用渲染引擎
                // 目前只是一个占位符
            }
            _ => {}
        }
    })?;
    
    Ok(())
}